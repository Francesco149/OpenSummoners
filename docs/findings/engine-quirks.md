...
> **The native capture proxy dismisses this dialog IN-PROCESS via INJECTION (ckpt 164).**
> An EXTERNAL dismiss can't work: a WSL-interop PowerShell can't grab the interactive
> desktop's foreground (`SetForegroundWindow` returns False), and this dialog ignores a posted
> `BM_CLICK` / `WM_COMMAND` / `WM_LBUTTONDOWN+UP` to the Launch button from a non-foreground
> external process — only a REAL foreground click, or an in-process message, advances it.  The
> old "drop `ddraw.dll`, let the exe load it, the proxy's harness dismisses post-load" scheme
> ALSO broke independently: the game loads `System32\ddraw.dll`, not an app-dir drop (ckpt
> 164), so the proxy never loaded at all.  **FIX:** `build/inject.exe` force-loads the proxy
> DLL by full path into the `CREATE_SUSPENDED` process, so the proxy is live BEFORE the main
> thread shows the dialog → the harness `installPeriodicWindowScan`-equivalent (`harness.h`)
> `PostMessage(BM_CLICK)`s the Launch button IN-PROCESS (works) and the capture is hands-free.
> (Confirmed: `[harness] clicked launch/OK button ('Launch')` then the game renders.)
...
retail.  **LEFT-facing: bank 0x8b mirrors with a UNIFORM `+152` flip (NOT `+4`) — the
renderer's facing==3 adds `flip_table[0x8b]`, so left = right + 152 for EVERY freeroam anim:
idle 0-2→152-154, walk 8-15→160-167, run 16-21→168-173, crouch 31/32→183/184, up 34/35→
186/187.  Set `ARCHE_FREEROAM_FLIP` 4→152 and the left pose/walk/idle/run all emerge — no
...
### #116 — the freeroam CAMERA FOLLOWS the party leader: each sim-tick `tgt = leader_pos − center_const (+ bias 0)`, clamped to `[0, map − viewport]`, then the easer (0x43d1d0) eases cur→tgt at +10/tick up to cap=800 — camera PINNED at the left edge until the leader passes screen ~300 (2026-06-22, ckpt 159, RE'd off 0x439690 + 0x510100, corroborated by sword2.osr)
The in-game loop sets the camera TARGET from the followed leader BEFORE calling the easer
(`0x439690:1101-1122`, right before the easer at `:1123`): `tgt_x = clamp(leaderX − 30000 + roomstate+0x50, 0, map_w − vp_w)`, `tgt_y = clamp(leaderY − 0x6a40 + roomstate+0x54, 0, map_h − vp_h)`, `cap = roomstate+0x58`, far-boost flag = 0.  The follow CONFIG (roomstate `+0x50`/`+0x54` bias, `+0x58` cap, `+0x5c` leader) is set by `0x510100:64-68` to **bias 0 / bias 0 / cap 800 / leader = FUN_00402f20()** (the party leader).  So the centering constants are **−30000 (X)** and **−0x6a40 (Y)** (= screen ~300/~272 when unclamped), bias **0**, cap **800**.  CORROBORATED bit-for-bit by sword2.osr: during a walk the background tiles stay FIXED (camera pinned, tgt clamps to 0) until Arche's `world_x > 30000` (screen 300), then they scroll and the camera eases at **+10/tick** (vel ramps 20,130,…,530 over the run, still climbing ⇒ cap 800 unsaturated) — and the floor tile sits at the SAME screen dx (32) in port + retail at the pinned phase (origin matches).  The Y camera never moves in the single-screen errands (`map_h − vp_h` clamps `tgt_y` to the entry origin).  Ported (ckpt 159): `freeroam_step` sets the target from `g_freeroam_char`; the existing `game_camera_step` easer eases it.  Before this the port's freeroam camera was STATIC (only the cutscene pans set a target) and Arche walked off-screen (USER studio note tick 2228).  `PORT-DEBT(freeroam-camera-config)`: the leader-pick + bias/cap stand in for the unported room-state follow-config subsystem (values RE'd off 0x510100; Arche is the lone party member here).

### #117 — the two brightness-ramp blend tables are ONE contiguous block: `ramp_b` base `DAT_008a9308` = `ramp_a` base `DAT_008a92b8` + 0x50 (one `PdBlend`), so `ramp_b[k]` IS `ramp_a[k+1]` — the same 80-byte descriptor (2026-06-23, ckpt 163, RE'd off the offsets + LUT-matched off sword2.osr)
Each blend descriptor (`PdBlend`, the per-channel shift/mask + the additive/alpha LUT) is **0x50 bytes** (`src/pixel_drawer.h` `_Static_assert sizeof(PdBlend)==0x50`).  The engine's two 20-entry ramp tables sit back-to-back in one allocation: `DAT_008a92b8` (`ramp_a`, group A) immediately followed by `DAT_008a9308` (`ramp_b`, group B) — and `0x8a9308 − 0x8a92b8 = 0x50` = exactly one descriptor.  So the tables OVERLAP by 19 entries: `ramp_b[k]` and `ramp_a[k+1]` are the **identical** descriptor (same bytes, same LUT).  This is why a particle whose decompile sets the blend to `ramp_b[18]` (`DAT_008a9308[18]`) renders byte-identically to `ramp_a[19]` (`DAT_008a92b8[19]`).  The sword-tip TRAIL (res 0x40b, `45e830` case 0x283f) is the case in point: sword2.osr shows every sparkle at one constant blend (LUT md5 `727d856f`), and a boot-time dump of the port's `g_ramp_a`/`g_ramp_b` pinned that LUT to **`g_ramp_a[19]`** — which is retail's `ramp_b[18]`.  **Port consequence:** the port's `g_pd_boot_group_a[20]` and `g_pd_boot_group_b[20]` are SEPARATE non-overlapping arrays (no shared-block aliasing), so a retail `ramp_b[k]` descriptor must be addressed as the port's `ramp_a[k+1]` (or `ramp_b[k]` won't match) — `sword_trail.c` emits the ramp_A index 19 (no `PARTICLE_PARAM8_RAMP_B` bit) to get retail's trail blend.  Pin any cross-referenced ramp index by LUT identity, not by the decompile's a-vs-b label alone.
