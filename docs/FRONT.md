...
- **LATEST (ckpt 124): the dialogue PORTRAITS are UN-MVP'd — the bust now RESOLVES per speaker (was a
  hardcoded wrong-character slot). `src/portrait.{c,h}` + the embedded face table; 982 host pass (+4);
  commit `ce1af81`. Montage on the feed (USER-requested).**
  1. **The bug.** The portrait was hardcoded to slot 663 = a WRONG character (head 100000104); every
     ...
     head-state (`+0x1d8`) + line `face_id` → the portrait pool-slot (`g_ar_sprite_slots` index).
  2. **The data.** `portrait_face_data.{c,h}` (NEW, generated by `tools/extract/portrait_face_table.py`)
     embeds the retail face table `DAT_006b6568` (147 records, 3 anim-state variants each). Speaker→head-state
     HARNESS-CAPTURED (`runs/portrait-gt`): Arche=100000101, Father=100000211, Mother=100000212.
  3. **VERIFIED (replay).** Father (green shirt) / Arche (headband girl) / Mother (brown-haired woman) busts
     each render correctly per line. The variant is harness-resolved: bVar4/var-C (the old MVP column) is
     FALSE (`in_ECX+0x2f0`==0); the A/B head-FACING is dynamic (captured A=8/B=10) so the static cast uses
     B — **PORT-DEBT(dialogue-portrait-facing)** (the per-line facing lands with the animated cast).
