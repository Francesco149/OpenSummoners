...
> structure the 18 KB dispatch writes into, port the small pure write units,
> defer the dispatch body (the ckpt-53/56/57 discipline).
>
> **RE — the runtime render grid.**  `FUN_00587e00`'s `in_ECX` is a ≳2.9 MB flat
> engine buffer — the *placed/decoded* form of the map (what `0x5a00c0` blits),
> NOT the parsed `map_data`.  Cells use a **fixed row pitch 0x80**:
> `idx = p1*0x80 + p2` (distinct from `map_data`'s z-major `(dim1*z+y)*dim0+x`).
> Layout (decoded from `587e00.c`, which addresses it via a `ushort*` base, e.g.
> `in_ECX[0x160818]` == byte 0x2c1030): front header `in_ECX[0..0x16]` + **4
> parallel per-cell regions** + a dim header —
> **A** @0x30 (0x40/cell = 4 sub-slots×0x10), **B** @0x140030 (0x10/cell),
> **C** @0x195030 (0xc/cell), **D** @0x2c1040 (2/cell); dims @0x2c1030
> (dim0/dim1/dim0·0xc80/dim1·0xc80).
>
> **PORT (pure, host-tested) `src/map_grid.{c,h}`:**
> - **`FUN_0054c970`** → `map_grid_clear_cell` (region C; pixel-dim bounds).
> - **`FUN_0058ca80`** → `map_grid_emit_obj` (fills a p3×p4 rows×cols block into
>   region B's 0x10-byte record + region D's u16; tile-dim bounds).
> - **`FUN_0058c910`** → `map_grid_emit_tile` (places a sprite-bank tile in a
>   region-A sub-slot; footprint = explicit span OR derived from the bank pixel
>   size `pool[bank]+0x20`/`+0x24` ceil-32 → tiles, clamped to the grid; the
>   bank pool `&DAT_008a760c` is an engine global → a `mg_bank_dims_fn` callback
>   keeps the port pure).
> - `map_grid_set_dims` ports the prologue's 4 dim-header writes so the
>   primitives run standalone.
> The 18 KB prologue's HUD/border bank-selection over the `DAT_008a76xx` pool +
> the front-header flags stay unported (the engine-coupled body of the rock).
>
> **STATE:** 6 host tests (`tests/test_map_grid.c`, bit-exact byte assertions) →
> **778 pass / 0 fail / 6 skip** (+6).  Ledger **187/1490 touched / 182 tested**
> (+3).  Both GUI builds compile clean; `map_grid.c` is in the `src` wildcard but
> not yet called by `main.c` (it is the *write* foundation the per-tile-id arms +
> the render read).  Full writeup: `docs/findings/in-game-intro.md`.  **NEXT:**
> the per-tile-id placement **arms** in `FUN_00587e00` (the recipes choosing
> which `map_grid_*` calls to make per town tile id — the `0x1b58b` family +
> `0x29ff4`, keyed on the cell shape selector `+0x10`), then a `0x5a00c0`
> read/blit slice, diff vs `runs/tas-ingame-1`.  Clean **/clear point**.
>
> ─────────────────────────────────────────────────────────────────────────────

# Session handoff — earlier (ckpt 57 — MAP CELL-RECORD SEMANTICS DECODED + pure cell accessors ported; `FUN_00587e00` SURVEYED = an 18 KB multi-checkpoint rock, NOT the 3 KB the ckpt-56 handoff guessed.  Opened the named-NEXT unit `FUN_00587e00` (map-data → world decoder) and found it is **18055 B / 3283 decompiled lines** — a per-tile-id placement dispatch deeply coupled to engine globals (`DAT_008a9b50`, the `DAT_008a76xx..86xx` bank-pointer pool) + ~8 helpers (`0x58ca80`/`0x58c910` emit, `0x58c8c0`/`8d0`/`cb30` layer pass, `0x54c970` grid-clear, `0x56df10`, `0x4118b0`).  Followed the ckpt-53/56 discipline: decode the data it consumes as re-runnable ground truth + port the small pure unit, defer the rock.  RE: read `FUN_00587970`'s map-descriptor object (dims `+0x20/24/28`, count `+0x2c`, cells `+0x34`, layer-hdr `+0x38`, layer sub-ptr table `+0x3c`) — confirms the ckpt-56 parser is structurally faithful — and `FUN_00587e00`'s per-cell loop (`587e00.c:586-601`), which reveals the **0x1c-B cell record**: `+0x04`=tile id (dispatch key), `+0x10`=footprint/orientation selector (0..0xc), `+0x0c`/`+0x14`/`+0x18`=placement params, `+0x00`=co-id, `+0x08`=aux; empty cell=all-zero; **linearization `idx=(dim1*z+y)*dim0+x`** (z-major).  GROUND TRUTH: extended `tools/extract/map_data.py` with `--cells` (per-z occupancy grid + tile-id/shape/field histograms); DATA 1022 = **160 of 5016 cells populated**, a coherent town backdrop (z=2 near=bottom ground strips, z=0 far=rooftop runs, z=1 mid details); tile ids cluster in the **`0x1b58b` family** + `0x29ff4`, shapes `{0,2,10,11,12,14,15}`.  **SCOPING WIN: every town tile id is `< 0x1bd82`**, so the giant `0x1bd82` autotile block + the `0x1d8ab`/`0x1ffbc…`/decoration switches are DEAD CODE for this map — the town backdrop port needs only the generic per-tile-id arms + `0x58ca80`/`0x58c910`.  PORT: pure host-tested `map_cell` + `map_data_cell(m,x,y,z)`/`map_data_cell_index` in `src/map_data.{c,h}` (the typed accessor the future `FUN_00587e00` port + render walk index into).  2 new host tests → **772 pass / 0 fail / 6 skip**.  Ledger **184/1490 unchanged** (data decode + accessors on the already-tested `0x587970`; `0x587e00` stays unported — I scrubbed the stray `FUN_` helper tokens out of the new comments so the derived ledger didn't drift).  Full writeup: `docs/findings/in-game-intro.md` "The cell record" + "DATA 1022 decoded" + "`FUN_00587e00` is an 18 KB multi-checkpoint rock".  NEXT: port `FUN_00587e00`'s per-tile-id placement arms for the ~9 town tile ids + the emit helpers (`0x58ca80`/`0x58c910`) writing the runtime grid, then the matching `0x5a00c0` slice to render the backdrop, diff vs `runs/tas-ingame-1`.  Clean /clear point.)

> **ckpt 57 — THE MAP CELL-RECORD FORMAT IS DECODED (ground truth) + the pure
> cell accessors are ported; `FUN_00587e00` is surveyed as an 18 KB
> multi-checkpoint rock.**  The handoff's named-NEXT (`FUN_00587e00`, "3282 B")
> turned out to be **18055 B / 3283 decompiled lines** — so this checkpoint did
> the ckpt-53/56 move: decode the data + port the small pure unit, defer the rock.
>
> **RE — the object + the cell record.**  `FUN_00587970`'s map-descriptor object
> (== `FUN_00587e00`'s `param_1`): dims `+0x20/24/28`, count `+0x2c`, cells ptr
> `+0x34` (n×0x1c), layer-hdr array `+0x38` (count×0x3c), layer sub-pointer table
> `+0x3c` (count×0x10 = 4 ptrs {a,b,c,d}/layer) — the ckpt-56 `map_data` parser is
> structurally faithful.  `FUN_00587e00`'s per-cell loop (`587e00.c:586-601`)
> reveals the **0x1c-byte cell record** + the **z-major linearization
> `idx=(dim1*z+y)*dim0+x`**: `+0x04`=tile id (dispatch key), `+0x10`=footprint/
> orientation selector (0..0xc), `+0x0c`/`+0x14`/`+0x18`=placement params,
> `+0x00`=co-id, `+0x08`=aux; an **empty cell is all-zero**.
>
> **GROUND TRUTH — DATA 1022 decoded (`map_data.py --cells`).**  The opening town
> = **160 of 5016 cells populated**, a coherent backdrop: **z=2** near plane (the
> long bottom ground/floor strips), **z=0** far plane (scattered rooftop runs),
> **z=1** a few mid details.  Tile ids cluster in the **`0x1b58b` family** (×78
> +×33+×19+…) + **`0x29ff4`** (×21); shapes used `{0,2,10,11,12,14,15}`;
> `+0x14`/`+0x18` always 0.
>
> **SCOPING WIN.**  Every town tile id is **`< 0x1bd82`**, so the giant `0x1bd82`
> autotile block + the `0x1d8ab`/`0x1ffbc…`/decoration switches in `FUN_00587e00`
> are **dead code for this map**.  The town backdrop port needs only the generic
> per-tile-id placement arms (each a recipe keyed on the shape selector) + the
> two emit helpers `0x58ca80`/`0x58c910` that write the runtime grid `in_ECX`.
>
> **PORT (pure, host-tested):** `map_cell` + `map_data_cell(m,x,y,z)` /
> `map_data_cell_index` in `src/map_data.{c,h}` — the typed accessor the future
> `FUN_00587e00` port + the render walk index into.  2 new host tests → **772
> pass / 0 fail / 6 skip**.  Ledger **184/1490 unchanged** (decode + accessors on
> the already-tested `0x587970`; `0x587e00` stays unported).  Full writeup:
> `docs/findings/in-game-intro.md`.  **NEXT:** the `FUN_00587e00` per-tile-id arms
> for the ~9 town tile ids + the emit helpers, then a `0x5a00c0` render slice,
> diff vs `runs/tas-ingame-1`.  Clean **/clear point**.
>
> ─────────────────────────────────────────────────────────────────────────────

# Session handoff — earlier (ckpt 56 — RUNTIME MAP-DATA LOAD PATH + FORMAT RESOLVED, and `FUN_00587970` PORTED + host-tested.  Surveying the next unit (`0x586010` sim → `0x5a00c0` render) showed both are multi-checkpoint rocks (the full engine sim; a self-contained scripted-scene render loop with its own GetTickCount pace machine + caption/sprite arrays + resource-unload teardown).  The tractable next data-layer unit is the **runtime MAP DATA**: the per-room VISUAL map (tilemap cell grid + 86 object/layer entries — the town backdrop) is **loaded from a PE DATA resource keyed by the room's SCENE index, sourced from the main EXE** (`DAT_008a6e7c` = the EXE module handle, NOT sotesd).  `0x586010:690` calls **`FUN_00587970(EXE, room.scene)`** → **`FUN_005b62a0`** = `FindResourceA(EXE, scene&0xffff, "DATA")` + LoadResource/LockResource → **`FUN_005b6340`** mode-1 sequential copy.  Opening town: room 210110 scene **1022** → **DATA resource 1022 in the EXE** (152,936 B, name **"MSD_SOTES_MAPDATA"**, **dims 88×19×3**, **86 layers**).  This REFINES plan 3a (ckpt 51 was incomplete): the res-probe hooked only the sprite decoder `0x5b7800`, so it never saw this separate FindResource path — "compiled-in" is true only of the ROOM REGISTRY (.rdata graph/names/scene-ids); the per-room visual map IS a loaded EXE resource.  Pairs with the EXE-NULL banks 0x570-0x572: the port loads both from the original `sotes.exe` as a datafile (`g_sotes_exe`).  PORT: new tool **`tools/extract/map_data.py`** (decode any map DATA resource; asserts EXACT consumption) + pure host-tested **`src/map_data.{c,h}`** porting `FUN_00587970`'s parse (caller supplies locked bytes; decodes magic+0x30 header+0x34 maphdr{name,dims,count} + `dim0*dim1*dim2`×0x1c cell array + `count` layer entries each 0x3c header + 4 sized sub-arrays into owned allocations; overrun-guarded).  4 new host tests → **770 pass / 0 fail / 6 skip**.  Ledger **184/1490** (`0x587970` now tested).  `map_data.c` is in the src wildcard, not yet wired into main.c (foundation for the FUN_00587e00 decoder).  Full writeup: docs/findings/in-game-intro.md "The RUNTIME MAP DATA".  NEXT: port **`FUN_00587e00`** (3282 B, the map-data → world decode reading this parsed structure) + the matching `0x5a00c0` render slice, diff vs `runs/tas-ingame-1`.  Clean /clear point.)
...
## ⭐ Current state (ckpt 60): title bit-exact loop; new-game scene bit-exact (box/text/cursor/tooltip + picker); prologue gem cutscene content bit-exact (63/64) + the fade-out timing ported. Milestone 1 (title+new-game+prologue) is 1:1 for rendered content. **Milestone 2 (the game proper) IN PROGRESS**: the in-game seam is wired + anchored (game_enter, both sides, rng-matched), the engine 0x59f2c0 is surveyed/decomposed, **plan 3a is RESOLVED** (town banks identified + boot batches g2/g3/g5 wired, title differ_px=0), **the `game_drive` scaffold is stood up** (ckpt 52, black map-load frame), the **static world tables are extracted** (ckpt 53), the **world-table layer is PORTED + host-tested** (ckpt 54) — `src/game_world.{c,h}` (registry build + `FUN_00585000` cross-reference + `FUN_00561c90` lookup) over generated `src/world_tables_data.{c,h}`, with the **`0x3f2`→opening-room resolution RESOLVED** (map 0x3f2 → key `0x334be` = room **210110 "Town of Tonkiness"**, scene 1022 — NOT Tolkien/110110), and (ckpt 55) the **MAP OBJECT is PORTED + host-tested** — `src/game_map.{c,h}` ports the `0x59f2c0` fresh-entry arm (the 0x4120 map object + 8 actor slots + header field init) + the `FUN_004c5350` room-key resolution (`map+0x4024=0x334be`), with `game_map_active_room` resolving end-to-end to room 210110 over the `game_world` layer. (ckpt 56) the **runtime MAP-DATA parser is PORTED** — `src/map_data.{c,h}` (`FUN_00587970`: the per-room visual map = DATA resource keyed by scene index, loaded from the EXE); (ckpt 57) the **cell record (0x1c B) + z-major linearization are decoded** + pure cell accessors ported (`map_data_cell`); (ckpt 58) the **runtime RENDER-GRID layout is decoded + its 3 WRITE PRIMITIVES are PORTED + host-tested** — `src/map_grid.{c,h}` (`FUN_0054c970`/`FUN_0058ca80`/`FUN_0058c910` + `map_grid_set_dims`): the ~2.9 MB flat grid (4 per-cell regions A/B/C/D + dim header, fixed 0x80 row pitch) that `FUN_00587e00` writes; (ckpt 59) the **`FUN_00587e00` per-tile-id placement DISPATCH (the 9 town arms) is PORTED** — `src/map_decode.{c,h}` (data → grid); (ckpt 60) the **static tilemap RENDER-WALK is LOCATED + its pure GEOMETRY ported** — `src/map_render.{c,h}` reads the decoded grid the way **`FUN_00490f30`** does (visible-window from the view/camera obj + grid dims; region-A sub-slot → draw-node geometry), CORRECTING the earlier model that `0x5a00c0` blits the tilemap (`0x5a00c0` is the caption/overlay player; `FUN_00490f30` is the backdrop renderer, draw nodes via `0x4917b0`). STILL UNPORTED (plan 3b): the `0x5a00c0` overlay player + `FUN_00490f30`'s engine-coupled draw machinery (sprite resolve 0x418470/0x417c40, palette tint DAT_008a93fc/0x4182d0, the `0x4917b0` draw-node pool enqueue @render_ctx+0x54 + the zdd blit/present, the region-C blend arms), the view/camera object construction, the `FUN_00587e00` 18 KB engine-coupled prologue, the sim 0x586010 (and the map object's opaque sub-inits 0x5612b0/0x5611d0/0x4e59a0). `game_world`/`game_map`/`map_data`/`map_grid`/`map_decode`/`map_render` are all in the src wildcard but NOT wired into main.c yet (the foundation those units read/write). Deferred: the EXE-NULL town banks 0x570-0x572 (confirmed in sotes.exe .rsrc; port loads via LoadLibraryExA(sotes.exe,AS_DATAFILE); registration coupled to engine-time slot indices, plan 3b); the fade-out frames' RENDER (box-alpha arm).
