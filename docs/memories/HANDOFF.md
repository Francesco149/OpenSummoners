...
where retail is a cooler greenish-gray (USER `osr_notes.jsonl` #4, tick 1726, crop [75,283,30,38]).

- **RE'd the code, not curve-fit (the dhash census + the decompile).**  The floor banks 0x187/0x188 (clones of
  the town floor res 0x769/0x76a PIXELS) carry an info-entry **+8 = &DAT_00675500** whose first u16 = **0x186**.
  **FUN_00417bc0** is the UNIFIED +8 consumer: first u16 == 0 ⇒ remap WITHIN the embedded palette (the NPC
  colour-variant body-bank shift, already ported as the equivalent pixel-index remap); != 0 ⇒ a CROSS-SLOT swap
  — copy palette entries from the slot whose POOL index == that u16.  0x186 = **res 0x76b** (a 32×32 palette-
  holder whose embedded palette is the cooler errands-floor colours).  So the SAME floor pixels render warm in
  the town (own palette) and grey in the errands (res 0x76b's) — a per-room PALETTE swap.  The port only
  modelled the within-palette case (identity for the floor) → no effect → wrong colour.
- **The full retail pipeline (0x490f30, the per-tile palette build):** per bank, embedded (`FUN_004178e0`) →
  FUN_00417bc0 (+8 swap) → FUN_004182d0 (+4 flag SCALE; NO-OP for the floor — flag 0 — and for town-area rooms
  where `DAT_008a93fc`=0x14 hits the case-switch default) → tone-curve grade → install.  So errands floor =
  grade(res 0x76b palette); the +4 scale is not exercised by any town/errands bank (a documented gap, below).
- **THE PORT (`asset_register.c`).**  `ar_apply_slot_palette_swap` ports the cross-slot half of FUN_00417bc0:
  in `ar_sprite_decode`, AFTER the NPC pixel-remap and BEFORE the grade (`title_sheet_format`), if the bank's
  +8 names a swap table (`AR_PAL_SWAP_TABLES`, generated by `tools/extract/info_palette_swap_tables.py`),
  overwrite the named palette entries with the source slot's RAW +0x34 palette via `ar_decode_slot_palette_raw`
  — NOT the channel-reordered `ar_palette_session_begin`/`bs_emit_palette_bgra`, so src/dst stay in the order
  the grade + 8bpp→display convert read.  Disjoint from the NPC remap (those tables have first u16 == 0).
  +host tests `cross_slot_palette_swap_tables` + `apply_slot_palette_swap_noop_paths`.
- **VERIFIED** off `port-walltint.osr` vs `retail.osr`: floor sheets res 1897 fr4/5/8 + res 1898 fr4/7/11 all
  `differ_px==0` (dhash byte-identical); the USER wall crop [75,283,30,38] at tick 1726 recon `differ_px==0,
  maxd==0`; town **58/58** + errands **73/73** shared sheets match (no regression).  `findings/errands-render-
  gaps.md` §3.
- **USER-VERIFY:** `osr_view.exe C:\oss-osr\port-walltint.osr C:\oss-osr\retail.osr` — scrub the errands
  (port ticks ~1690+): the floor/wall is the cooler greenish-gray, matching retail.  Feed: the PORT|RETAIL|DIFF
  wall+floor montage (floor dark = match).
- **DOCUMENTED GAP (not the floor):** the +4 flag SCALE (FUN_004182d0, the per-scene tint cases) and the 19
  OTHER within-palette +8 tables (banks not spawned in town/errands) are unported — inert for the reached
  scenes (the floor's flag is 0 and town-area `DAT_008a93fc`=0x14 short-circuits the scale).  When a non-town
  room with a real tint variant (DAT_008a93fc 0..5) is reached, port FUN_004182d0 + plumb DAT_008a93fc.
...
1032 host pass).**  The port showed a black empty recess where retail has a roaring fire (USER `osr_notes.jsonl`
#3, tick 1726).
...
- **THE FADE (USER notes #6/#7) — DONE.**  The house was hard-cutting to the errands.  RE'd off `0x4d7d80:1203-
  ...
  (1668 — refined from 1670 off the fine per-tick brightness curve, retail cover first-darkens 1669) + the box
  LINGERS over the cover (`cutscene_room.exit_box_hold`=10).  Now the cover darkens TICK-FOR-TICK with retail
  (1669-1689 within 1-2 luma; cover-region differ 30k→~4k).  Also fixed (USER note #16) a stray errands speech
  bubble = the house box_linger box froze half-shrunk in `cs->closing` at chain-complete; cs_finish_beats now
  clears cs->closing on complete.  Commits `2a464eb`+`6108ea4`.  NAV-BUILD CAVEAT for dialogue_timeline: a
  cover-followed last line's nav tick = its confirm (~box_hold ticks BEFORE its measured "adv").
...
`retail.osr` tick-for-tick.  USER course-correction this ckpt: *trace the exact retail code, don't measure
choreography and bake a constant* (memory [[re-not-measure-choreography]]) — so the beats are RE'd from the
decompile, not curve-fit.  1023 host pass (+2).  Engine-quirk #109.

- **The mechanism (quirk #109): `0x4d7d80` is a FLAT beat sequence** the beat-runner `0x439690` pumps —
  dialogue lines interleaved with camera pans / actor run-offs / wait timers / scene fades, each blocking
  (`0x439680` returns 6 = not-done) until it completes.  Ported a beat model to `cutscene.{c,h}`
  (`cutscene_beat` WAIT / CAMERA_PAN / FADE; a per-room SPARSE `cutscene_line_lead` map + `exit_beats`; a
  beat sub-phase in `cutscene_step` that closes the box + runs the beats; one-shot `cutscene_action` the
  caller performs).  Three beats:
  - **L7→L8 "Arche runs ahead":** a FIRE-AND-FORGET camera pan to (28000,12800)@400 (the `0x43d1d0` easer —
    `0x439690:623-641` sets the view tgt then clears `in_ECX[0x13]`; draw-verified −148px scroll) + the
    case-4 RUN-OFF wait.  KEY RE: `0x402730` (the run-off) does `*(in_ECX+0x20)=4` — it OVERWRITES the beat
    type to 4, so the `0x439680` at `:229` is case 4 (`0x439690:1137` waits for the actor-beat pool to clear),
    NOT case 3 (camera).  The run-off duration (97t = the MEASURED 167t L7adv→L8-first-glyph gap − wait50 −
    the ~20t box pop-in) is the actor MOVE stepper `0x54f980` = the cast, a tagged `cutscene-party-chars`
    stand-in.
  - **Arrival EXIT:** wait20 + a fade-TO-black (cover) that GATES the room-key swap (the reload happens UNDER
    full black — no early snap, note #5/#6).
  - **House ENTRY:** a fade-FROM-black reveal + wait50 (note #7).  The fades RE the arm `0x439690:555-563`
    (MODE = `in_ECX[10]`: 1 reveal/_OUT, 2 cover/_IN; SPEED 1000; VARIANT = LCG `(rand*3)>>15` — main.c draws
    it at the wire site).  A FADE beat completes when the live `scene_fade` grid SETTLES (`cutscene_set_fade_active`
    = retail's case-2 grid-done gate).
...
and 314/323 ticks of (speaker name, revealed body) are identical.  1017 host pass (+5).

**The diagnosis (the high-value finding): the port's reveal/skip MECHANICS were ALREADY faithful.**
`dialogue_timeline.py` read the reveal curve straight off `retail.osr` (counting the body MAIN glyphs,
colour `0x3e537d`, per tick) and proved retail types **1 char / 5 ticks** (space = 1 tick) and the skip is an
**instant `reveal→total` jump** — EXACTLY the port's existing model.  So note #4 ("port reveals text earlier
after X/Enter") was a CADENCE artifact (the `0x24`-SPAM nav skipped every line the instant it armed and raced
ahead), NOT a reveal-rate bug.  Two cadence gaps remained, both fixed:

**What landed (5 commits):**
- **Tick-keyed input-trace (`input_trace.{c,h}`).** An entry may key on the deterministic SIM-TICK
  (`{"tick":N,"ids":[...]}`) instead of the Flip frame — the axis the trace-studio tick-join shares, so a
  tick-keyed confirm fires at the SAME sim-tick on both sides (the port's Flip cadence differs from retail's,
  so a Flip-keyed confirm can't align the dialogue).  Axis is PER-ENTRY → one nav MIXES a flip-keyed boot
  prefix (the menus, while sim_tick is still 0) + tick-keyed in-game confirms.  `input_trace_replay` takes
  `sim_tick` and picks the counter per entry; `main.c feed_input` passes `g_sim_tick_count`.
- **The box re-pop model (`dialogue.{c,h}` + `cutscene.{c,h}`).** The port re-armed (full 20-update pop-in)
  on EVERY line.  Retail keeps the box OPEN across a SAME-speaker advance (`dialogue_set_text`: reset only
  the typewriter, gap 1t — retail L3→L4→L5 all Arche) and on a SPEAKER CHANGE re-opens from HALF scale
  (`dialogue_reopen`, `DIALOGUE_REOPEN_SCALE=500` → ~10-update reopen, content at advance+11t = retail's
  exact reopen, vs the first box's ~20-update slide-in).  `cutscene_step` picks the arm mode per advance:
  ARM_OPEN (first line) / ARM_REOPEN (speaker change) / ARM_KEEP (same speaker, by `name_va` within a room).
- **Keep the word-wrap space (`dialogue_expand_text`).** The expander consumed the space at a wrap
  ("our new"/"hometown"); retail RENDERS the trailing space (retail.osr y=196 row "...our new " ends in a
  space).  Keeping it makes the body byte-identical; the space's grade is 1 and sits past every line's skip,
  so the cadence is unchanged.
...
- **THEME 3 — the arrival→house TRANSITION CHOREOGRAPHY (`cutscene-beat-runner`).**  Between arrival L7
  ('Cool!', advance tick 982) and L8 ('Mom, Dad, c'mon!', retail tick 1149) retail plays a **167-tick scripted
  beat** — Arche RUNS to the house (note #5, the actor run-off `0x402730`/`0x401e60`); the room load is gated
  on it; then a fade-from-black / inverted-iris REVEAL on house entry (#6/#7, the `scene_fade` the port lacks
  on `reload_room_backdrop`).  The port SNAPS (advances L7→L8 instantly).  This is the divergence after tick
  982 (port L8 at 983 vs retail 1149).  PORT the run beat + gate the room load + the entry fade.
- **THEME 2 — the cutscene CAST + ambient render (the most involved).**  Butterfly colour variant
  (`butterfly-wander` frame_base 0/4/8/12, note #0), NPC colour variant (#1, `effect-color-variant`),
  butterfly flutter (#2), the static cast animation frame (#3, `cutscene-party-chars` — the party-band render
  `0x4997b0`).
- **THEN the FREEROAM HAND-OFF** (behind the 1:1 bar): at the errands room (`0x334dc`) stop the sequencer +
  run `character_step` on the live `axis_held` (`+0x200==0` char-AI; mover DONE, quirk #103).
...
ports `FUN_0049c640` over the `FUN_00490b90` world→screen projection; the box rides the speaker's world pos
projected through the LIVE camera.  1011 host pass (+1).  Quirk #106; fresh port `.osr` at
`C:\oss-osr\port-boxpos.osr`.

**What landed:**
- **Captured EVERY `0x49c640` input from retail first** (`tools/flow/box_pos_inputs_fields.json`,
  `runs/box-pos-inputs`).  Added a new agent read kind `argchain` (arg-rooted pointer hops, like `thischain`
  from ECX) to reach the speaker body `*(*(arg5+0x40)+off)`.  PROVED the formula AND that the port's
  `mr_camera` IS `0x490b90`'s `in_ECX` (same `off34/4c/5c/60/64/68/74`) → the projection is a free reuse.
- **The formula (quirk #106):** `scr_x = wx/100 - cam_x/100` (two truncating /100), `scr_y = wy/100 -
  (cam_y + scroll·100)/100`; `box_x = clamp((sprite_w/200 - W/2) + scr_x, 0x20, 0x260-W)`;
  `box_y = (metric_14/100 + spk+0x1c) - H - 0x30 + scr_y`.  Both clamps fire in real data (box_x 32 left /
  200 right); the flip-below branch is dead in the town intro.
- **Port:** `dialogue_box_position` (pure C, `dialogue.c`; host-tested bit-exact vs all 10 captured cases +
  the centered fallback).  `cutscene.c` bakes per-line speaker world pos (the cast MOVES: Arche runs ahead
  at arrival L9 → world 73104) + per-character body geometry (`BODY_ARCHE`/`BODY_ADULT`; Arche off_1c=-8 →
  her box drops 8px).  These are HARNESS-CAPTURED because the live cast is PORT-DEBT(cutscene-party-chars).
  `game_render_dialogue` projects through the live `g_game_camera_mr` each frame; `dialogue_scaled_rect` now
  takes the anchor.  Port-side annotation: `CALL_TRACE_BEGIN(0x49c910)` emits the computed box_x/box_y.
- **Verified:** a port `--call-trace` over `nav-zspam-ext.jsonl` reproduced retail's box for arrival L1-L9 +
  ALL house lines BIT-EXACT.  The ONE residual — arrival L10 Mother, port (62,148) vs retail (32,148) — is
  retail's camera panning to 28000 (following Arche running ahead at L9) while the port's static cast keeps
...
**The USER drove a frame-by-frame trace-studio pass; FOUR dialogue-rendering bugs fixed** (commits `e8d5c0b`
errands floor / `1d826c3` house+errands props / `cbbab94` the dialogue-portrait +13 pool-vs-array offset).
Full detail in `PROGRESS.md`; the box-POSITION gap that pass left is closed above (ckpt 132).
