...
## ⭐ Current state (ckpt 54): title bit-exact loop; new-game scene bit-exact (box/text/cursor/tooltip + picker); prologue gem cutscene content bit-exact (63/64) + the fade-out timing ported. Milestone 1 (title+new-game+prologue) is 1:1 for rendered content. **Milestone 2 (the game proper) IN PROGRESS**: the in-game seam is wired + anchored (game_enter, both sides, rng-matched), the engine 0x59f2c0 is surveyed/decomposed, **plan 3a is RESOLVED** (town banks identified + boot batches g2/g3/g5 wired, title differ_px=0), **the `game_drive` scaffold is stood up** (ckpt 52, black map-load frame), the **static world tables are extracted** (ckpt 53), and (ckpt 54) the **world-table layer is PORTED + host-tested** — `src/game_world.{c,h}` (registry build + `FUN_00585000` cross-reference + `FUN_00561c90` lookup) over generated `src/world_tables_data.{c,h}`, with the **`0x3f2`→opening-room resolution RESOLVED** (map 0x3f2 → key `0x334be` = room **210110 "Town of Tonkiness"**, scene 1022 — NOT Tolkien/110110). The engine's runtime — the 0x4120 map object + 8 actor slots, the sim 0x586010, the render 0x5a00c0 — remains UNPORTED (plan 3b; see the ckpt-54 block + docs/findings/in-game-intro.md). `game_world` is not yet wired into main.c (it's the foundation those units read). Deferred: the EXE-NULL town banks 0x570-0x572 (confirmed in sotes.exe .rsrc; port loads via LoadLibraryExA(sotes.exe,AS_DATAFILE); registration coupled to engine-time slot indices, plan 3b); the fade-out frames' RENDER (box-alpha arm).
