...
| `render-palette-tint` | `map_render`/`draw_pool`: the per-sprite palette tint (`DAT_008a93fc`/`0x4182d0`) is not applied — geometry only.  Also `parallax` (ckpt 66): retail selects the far-plane cel via the palette-aware `0x417c40`; the port uses the plain frame getter `0x418470` (as the tiles do), so the sky/mountain render with the base palette (no time/difficulty tint). | Recolors pixels, not draw-list geometry; the geometry is provable without it. | the present pass blits actual pixels and the tint affects the diff. |
