...
| body divergence at sword2 tick 2082 (−9px) + the dashes (~+20/+35/+58px) | **RESOLVED (ckpt 165) — the brake-stop is a REAL 1-tick brake-onset bug, NOT aliasing** | The ckpt-163g "camera-hidden aliasing" diagnosis was WRONG.  The state-ful re-drive (sim-tick-axis injection + brute-force handoff, this ckpt) put retail's wx/hvel on the table: the **walk ACCEL is bit-exact** (14 ticks 0-divergence), but the port's **BRAKE starts 1 sim-tick early** — retail holds full velocity on the release tick and brakes the next tick; the port brakes on the release tick (constant −240 wx = −2.4px lag).  Root cause `character.c:75-81` (release sets cmd_dir=0 immediately; no release latency).  Fix candidate + RE-to-do: `findings/freeroam-brake-onset.md`.  The dashes are not yet re-tested (the synth double-tap needs explicit RING edges). |
