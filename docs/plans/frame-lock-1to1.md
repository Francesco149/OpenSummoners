...
| body divergence at sword2 tick 2082 (−9px) + the dashes (~+20/+35/+58px) | **RESOLVED (ckpt 165) — walk accel BIT-EXACT; the brake/run "+1" is a HARNESS artifact, the port is CORRECT** | The state-ful re-drive (sim-tick-axis injection + brute-force handoff) put retail's wx/hvel on the table: the **walk ACCEL is bit-exact** (14 ticks 0-divergence).  The brake/run looked "1 tick early" in the port, but USER-directed deep RE (decompile + the unpacked-exe assembly) proved retail's input→command→velocity is **single-frame, zero latency** — the reduction `0x478ba0` (this=actor) writes `actor+0x14854`, the apply `0x442a70` (this=actor) reads it, phase-A→B same frame.  So real retail brakes on the release tick, **exactly like the port**.  The re-drive's +1 is the proxy injection labeling (pre-bump inject vs post-bump emit/record), absorbed by the warmup on the press but exposed on the no-warmup brake/run.  **No port fix.**  **HARNESS FIX LANDED (ckpt 166):** the inject-side structural +1 (`engine_input.h EI_TICK_AXIS` = fire tick entries at the pending emit label `g_eh_sim_tick+1`, matching `feed_input`) — brake onsets now re-drive BIT-EXACT (`retail-harnessfix.osr` brake 2160 == port).  It EXPOSES a PROXY held-injection warmup residual (proxy walk-press 1 idle tick vs the GT's 2). **ckpt 168 CORRECTION: this was a MISDIAGNOSIS — the proxy was FAITHFUL, the PORT was 1 tick too long.** RE'd `0x46a880`/`0x468a20`/`0x478ba0` (the level fill → rising-edge ts stamp → 11 ms gate) + instrumented the proxy state pass with the engine's OWN input fields (`cmd0`/`lvlR`/`edgeR`); a clean tick-axis re-drive (`retail-decomp.osr`) MEASURED retail's warmup = **exactly 1 idle tick** (press tick: lvlR 0→1 + edge stamped, cmd0 still 0; +1 tick: cmd0=2 + motion).  The frida flip-axis GT (`capdash2`) carried a +1 (held entry "flip N" lands at frame N+1's poll — agent advances on `g_flip_frame`, bumped at the PRESENT after the poll); the port's `DELAY=3` was tuned to it.  Fix: `CHAR_INPUT_REPEAT_DELAY` 3→2; `port-decomp.osr` vs `retail-decomp.osr` BIT-EXACT (idle through the press tick, motion +1, ramp byte-identical).  **The `0x46a880` edge/press_ts thread is now CLOSED — the re-drive is faithful for warmup-anchored onsets.**  `findings/freeroam-brake-onset.md`. |
