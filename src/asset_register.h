 ...
 *
 *   4. **9 FUN_00582b80 calls + 1 FUN_00582d00** at the tail —
 *      another SS_MGR thiscall pattern that copies fields from a "this"
 *      slot into a target slot.  Plus a final 5-element memcpy loop
 *      (20 B per slot) into a +0xae0 region.  **DEFERRED**.
 *
 * The deferred subsystems write to slot fields and parallel tables that
 * no consumer code we've ported reads.  Downstream impact: pixel-drawer
 * and the title-scene boot path observe `g_ar_sprite_slots[79..423]` as
 * fully populated with the right (rid, w, h, ck, sf, type) tuples —
 * which is what the slot consumers we know about actually read.  The
 * flag-table and parallel-pool reads happen in functions we haven't
 * studied yet (likely the sprite-frame-variant rendering path).
 *
 ...
 * the 233 sprite-slot register operations (91 inlined + 142 helper-
 * style).  The function's remaining ~480 operations (slot clones,
 * parallel-info-table writes, FUN_00582b80 calls) are deferred — see
 * the ar_register_group3_sprites docstring for the full breakdown.
 * No consumer of those deferred writes is ported yet, so this slot-
 * register subset is what downstream code can observe today. */
