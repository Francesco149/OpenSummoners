 ...
 *      a "this" slot into a target slot.  Primitives are PORTED
 *      (`ar_sprite_slot_clone` + `ar_info_entry_clear`).  The
 *      call-cluster wiring (which slot ⇄ which info-entry pair) is
 *      still DEFERRED — but the trailing 5-element struct copies into
 *      pool[257..261] are part of the 4th-pass event table above
 *      (STRUCT_COPY events).
 *
