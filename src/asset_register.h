...
/* The NPC colour-variant palette-index remap (THEME 2 #1): given the retail
 * .rdata VA an info-entry's +8 (->data) holds, return the 256-entry remap that
 * recolours a cloned variant bank's 8bpp pixels (NULL = not a colour variant).
 * Applied by ar_sprite_decode to the 8bpp sheet before the slice.  Data
 * generated by tools/extract/npc_palette_remap.py.  See findings/npc-colour-variant.md. */
const unsigned char *ar_npc_palette_remap(uint32_t rdata_va);

/* A CROSS-SLOT palette-swap table (FUN_00417bc0 with the +8 table's first u16
 * != 0): replace palette entries [lo,hi] with the SAME-index entries of the slot
 * whose pool index == src_pool.  All observed tables are contiguous identity
 * copies; data generated by tools/extract/info_palette_swap_tables.py. */
struct ar_pal_swap_table {
    uint32_t rdata_va;          /* the info-entry +8 .rdata VA that keys it */
    uint16_t src_pool;          /* source slot pool index (table's first u16) */
    uint8_t  lo;                /* first palette index replaced (inclusive) */
    uint8_t  hi;                /* last palette index replaced (inclusive) */
};

/* Look up the cross-slot swap table for a +8 .rdata VA (NULL = none — a
 * within-palette NPC table or an unrelated VA). */
const struct ar_pal_swap_table *ar_pal_swap_table_for_va(uint32_t rdata_va);

/* FUN_00417bc0 (cross-slot half): if `slot`'s info-entry +8 names a CROSS-SLOT
 * palette-swap table (src u16 != 0 — e.g. the errands floor reads res 0x76b's
 * cooler palette), overwrite the named entries of `palette` with the source
 * slot's RAW embedded palette.  No-op for the within-palette NPC tables (handled
 * on the pixels).  Applied by ar_sprite_decode BEFORE the grade — matching retail
 * 0x490f30 (embedded → swap → grade → install).  findings/errands-render-gaps.md §3. */
...
/* Re-apply only the DATA_SET info events after the clones (which clear info +8);
 * restores the NPC colour-variant palette-remap pointers.  See the .c comment. */
