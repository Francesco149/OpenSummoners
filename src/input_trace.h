...
struct input_trace_entry {
    uint32_t frame;                        /* Flip frame the ids fire at  */
 ...
 * whitespace, `# …` comment lines.  Numbers may be decimal or 0x-hex.  Frames
 * must be non-decreasing (out-of-order fails).  `len` is the byte length; an
 * embedded NUL is tolerated. */
...
/* Inject every not-yet-fired entry whose frame <= `present_frame` into `mgr`'s
 * ring as fresh presses stamped `now`, advancing the internal cursor.  Each id
 * takes the next ring slot round-robin (mgr->ring[head] must point at a real
 * record — the drive's input_mgr satisfies this).  No-op when the trace is
 * empty, fully consumed, or `mgr` is NULL.  Call once per frame, before the
 * scene step's poll. */
