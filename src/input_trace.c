/*
 * src/input_trace.c — port-side deterministic input replay (see input_trace.h).
 *
 * A narrow hand-rolled parser for the harness's `{"frame": N, "ids": [..]}`
 * JSONL, plus a ring-injection replay pump.  No real JSON library: the format
 * is fixed and small, and keeping it dependency-free matches the rest of the
 * port (the harness side that writes these files is tools/frida_capture.py).
 */

#include "input_trace.h"

#include <stdio.h>
#include <stdlib.h>
#include <string.h>

void input_trace_free(struct input_trace *t)
{
    if (t == NULL) return;
    free(t->entries);
    t->entries   = NULL;
    t->count     = 0;
    t->cap       = 0;
    t->cursor    = 0;
    t->ring_head = 0;
}

/* ─── tiny scanner over [p, end) ─────────────────────────────────────── */

static void skip_ws(const char **pp, const char *end)
{
    const char *p = *pp;
    while (p < end && (*p == ' ' || *p == '\t' || *p == '\r' || *p == '\n'))
        p++;
    *pp = p;
}

/* Skip blanks within a line (not newlines). */
static void skip_inline_ws(const char **pp, const char *end)
{
    const char *p = *pp;
    while (p < end && (*p == ' ' || *p == '\t')) p++;
    *pp = p;
}

/* Parse a decimal or 0x-hex integer at *pp into *out.  Returns 1 on success
 * and advances *pp past the digits; 0 if no number is present. */
static int parse_number(const char **pp, const char *end, long *out)
{
    char buf[32];
    const char *p = *pp;
    size_t n = 0;

    skip_inline_ws(&p, end);
    if (p < end && (*p == '-' || *p == '+')) {
        if (n < sizeof buf - 1) buf[n++] = *p;
        p++;
    }
    while (p < end && n < sizeof buf - 1 &&
           ((*p >= '0' && *p <= '9') ||
            (*p == 'x' || *p == 'X') ||
            (*p >= 'a' && *p <= 'f') || (*p >= 'A' && *p <= 'F'))) {
        buf[n++] = *p++;
    }
    if (n == 0) return 0;
    buf[n] = '\0';

    char *endp = NULL;
    long v = strtol(buf, &endp, 0);   /* base 0 → handles 0x prefixes */
    if (endp == buf) return 0;
    *out = v;
    *pp = p;
    return 1;
}

/* Expect a specific char at *pp (after inline whitespace).  Consumes it on
 * match. */
static int expect(const char **pp, const char *end, char c)
{
    skip_inline_ws(pp, end);
    if (*pp < end && **pp == c) { (*pp)++; return 1; }
    return 0;
}

/* Parse a quoted key at *pp and report which one via *kind:
 * 0 = "frame", 1 = "ids", -1 = unknown/error.  Advances past the closing
 * quote.  Returns 1 on a well-formed quoted key, 0 on a malformed token. */
static int parse_key(const char **pp, const char *end, int *kind)
{
    if (!expect(pp, end, '"')) return 0;
    const char *p = *pp;
    const char *start = p;
    while (p < end && *p != '"') p++;
    if (p >= end) return 0;
    size_t klen = (size_t)(p - start);
    p++;                                  /* past closing quote */
    *pp = p;

    if (klen == 5 && memcmp(start, "frame", 5) == 0)      *kind = 0;
    else if (klen == 3 && memcmp(start, "ids", 3) == 0)   *kind = 1;
    else                                                  *kind = -1;
    return 1;
}

/* Parse `[a, b, …]` of numbers into e->ids/n_ids.  Returns 1 on success. */
static int parse_id_array(const char **pp, const char *end,
                          struct input_trace_entry *e)
{
    if (!expect(pp, end, '[')) return 0;
    e->n_ids = 0;
    skip_inline_ws(pp, end);
    if (*pp < end && **pp == ']') { (*pp)++; return 1; }   /* empty array */

    for (;;) {
        long v;
        if (!parse_number(pp, end, &v)) return 0;
        if (e->n_ids >= INPUT_TRACE_MAX_IDS) return 0;      /* too many ids */
        e->ids[e->n_ids++] = (int32_t)v;
        skip_inline_ws(pp, end);
        if (*pp < end && **pp == ',') { (*pp)++; continue; }
        if (*pp < end && **pp == ']') { (*pp)++; return 1; }
        return 0;                                            /* malformed */
    }
}

/* Parse one `{"frame":N, "ids":[..]}` object at *pp into *out.  Keys may be in
 * any order; an absent "ids" yields n_ids == 0.  Returns 1 on success. */
static int parse_entry(const char **pp, const char *end,
                       struct input_trace_entry *out)
{
    memset(out, 0, sizeof *out);
    int have_frame = 0;

    if (!expect(pp, end, '{')) return 0;
    skip_inline_ws(pp, end);
    if (*pp < end && **pp == '}') { (*pp)++; return have_frame; }

    for (;;) {
        int kind;
        if (!parse_key(pp, end, &kind)) return 0;
        if (!expect(pp, end, ':')) return 0;

        if (kind == 0) {                                    /* frame */
            long v;
            if (!parse_number(pp, end, &v)) return 0;
            out->frame = (uint32_t)v;
            have_frame = 1;
        } else if (kind == 1) {                             /* ids */
            if (!parse_id_array(pp, end, out)) return 0;
        } else {
            return 0;                                        /* unknown key */
        }

        skip_inline_ws(pp, end);
        if (*pp < end && **pp == ',') { (*pp)++; continue; }
        if (*pp < end && **pp == '}') { (*pp)++; break; }
        return 0;
    }
    return have_frame;
}

static int push_entry(struct input_trace *out, const struct input_trace_entry *e)
{
    if (out->count >= INPUT_TRACE_MAX_ENTRIES) return 0;
    if (out->count == out->cap) {
        size_t ncap = out->cap ? out->cap * 2 : 16;
        struct input_trace_entry *n =
            (struct input_trace_entry *)realloc(out->entries, ncap * sizeof *n);
        if (n == NULL) return 0;
        out->entries = n;
        out->cap = ncap;
    }
    out->entries[out->count++] = *e;
    return 1;
}

int input_trace_parse_buf(const char *buf, size_t len, struct input_trace *out)
{
    memset(out, 0, sizeof *out);
    if (buf == NULL) return 0;

    const char *p = buf;
    const char *end = buf + len;
    uint32_t last_frame = 0;
    int have_prev = 0;

    while (p < end) {
        skip_ws(&p, end);
        if (p >= end) break;
        if (*p == '#') {                                    /* comment line */
            while (p < end && *p != '\n') p++;
            continue;
        }
        struct input_trace_entry e;
        if (!parse_entry(&p, end, &e)) return 0;
        if (have_prev && e.frame < last_frame) return 0;    /* out of order */
        last_frame = e.frame;
        have_prev = 1;
        if (!push_entry(out, &e)) return 0;
        /* Tolerate trailing junk up to the newline (e.g. a trailing comma in
         * the harness's pretty output is not produced, but be lenient). */
        while (p < end && *p != '\n') {
            if (*p != ' ' && *p != '\t' && *p != '\r' && *p != ',') return 0;
            p++;
        }
    }
    return 1;
}

int input_trace_load(const char *path, struct input_trace *out)
{
    memset(out, 0, sizeof *out);
    if (path == NULL) return 0;

    FILE *f = fopen(path, "rb");
    if (f == NULL) return 0;

    if (fseek(f, 0, SEEK_END) != 0) { fclose(f); return 0; }
    long sz = ftell(f);
    if (sz < 0) { fclose(f); return 0; }
    if (fseek(f, 0, SEEK_SET) != 0) { fclose(f); return 0; }

    char *buf = (char *)malloc((size_t)sz + 1);
    if (buf == NULL) { fclose(f); return 0; }
    size_t got = fread(buf, 1, (size_t)sz, f);
    fclose(f);
    buf[got] = '\0';

    int ok = input_trace_parse_buf(buf, got, out);
    free(buf);
    return ok;
}

void input_trace_replay(struct input_trace *t, uint32_t present_frame,
                        input_mgr *mgr, uint32_t now)
{
    if (t == NULL || mgr == NULL) return;

    while (t->cursor < t->count && t->entries[t->cursor].frame <= present_frame) {
        const struct input_trace_entry *e = &t->entries[t->cursor];
        for (uint16_t i = 0; i < e->n_ids; i++) {
            input_event *rec = mgr->ring[t->ring_head];
            if (rec != NULL) {
                rec->id   = e->ids[i];      /* fresh press */
                rec->flag = 1;
                rec->ts   = now;
            }
            t->ring_head = (t->ring_head + 1) % INPUT_RING_LEN;
        }
        t->cursor++;
    }
}
