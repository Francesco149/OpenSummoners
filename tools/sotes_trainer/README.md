...
| `dlgskip` | `on`(bool) | auto-advance an OPEN dialogue hands-free. PASSIVELY reads the SE dialogue widget (`*(input_mgr+0x374)!=0` ⇒ a box is on screen) and injects the advance ids (`0x24`/`0x27`) **only while a box is up** (freeroam injects NOTHING). ⚠ but those ids double as world ACTION input, so it auto-CONFIRMS a world prompt that IS open (bed/door — e.g. once mouse-fly lands you on the bed). **Default OFF** — the world-safe auto-skip is `autoskip`; turn `dlgskip` on only when you want open prompts auto-advanced. |
