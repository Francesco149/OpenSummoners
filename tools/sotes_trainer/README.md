...
| `fastskip` | `on`(bool) | force the active dialogue line's typewriter reveal to total each tick (instant text) — a pure UI-state write (no button ⇒ door-safe). Walks `g_dlg_grid → widget → TM+0x14=TM+0x10` (reveal counter RE'd session 8, DESIGN). Pair with `dlgskip` to fully fast-forward. Default OFF. Chain VERIFIED live; on-screen visual pending a foregrounded run. |
